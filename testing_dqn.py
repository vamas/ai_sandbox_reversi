...
if __name__ == "__main__":
    ...
    for level in range(0, 5):
        model_b.load_state_dict(torch.load("{}dqn_model_full_40000_{}.pth".format(models_path, level), weights_only=True))
        model_w.load_state_dict(torch.load("{}dqn_model_full_40000_{}.pth".format(models_path, level), weights_only=True))
        black_agent = DQNAgent(Player.BLACK, model_b, torch_device=get_device())
        white_agent = DQNAgent(Player.WHITE, model_w, torch_device=get_device())

        # against opponent level
        opponent_level_results = []
        for opponent_level in range(0, 5):
            ...
            for agent in [black_agent, white_agent]:
                ...
                white = DQNAgent(Player.WHITE, opponent_model, torch_device=get_device()) if agent.player == Player.BLACK else white_agent
                black = DQNAgent(Player.BLACK, opponent_model, torch_device=get_device()) if agent.player == Player.WHITE else black_agent

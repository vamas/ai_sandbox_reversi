...
async def main():
    ...
    print("MPS is available: {}".format(torch.backends.mps.is_available()))  # Should print: True®
    print("MPS is built: {}".format(torch.backends.mps.is_built()))  # Should print: True

    metric_logger_task = asyncio.create_task(periodic_logger_task(log_interval))
    # metric_logger_task.add_done_callback(handle_task_result)

    for i in range(1):
        await train(batch_size, discount_factor, epsilon, epsilon_min, hidden_dim, learning_rate, memory_size, reward_decay,
                total_games)
    await asyncio.sleep(10)
    metric_logger_task.cancel()
